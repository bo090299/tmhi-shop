...
class Appointment(models.Model):
    ...
    def __str__(self):
        return f"{self.customer_name} - {self.stylist.name} - {self.appointment_date}"

class Itmerkez(models.Model):
    itname = models.CharField(max_length = 30)
    servicer = models.TextField()
    image  = models.ImageField(upload_to = 'itcenter/')
    phone_number = models.CharField(max_length = 20, unique = True)

    def __str__(self):
        return f"{self.itname} - {self.servicer}"