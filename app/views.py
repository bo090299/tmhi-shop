...
def salon(request):
    ...
    if category:
        stylists = Stylist.objects.filter(category=category)
    else:
        stylists = Stylist.objects.all()
    return render(request, 'salon.html', {'stylists': stylists})



from django.db.models import Q

def appointment(request, stylist_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Ulanyjy tarapyndan girilen maglumatlar
            customer_name = form.cleaned_data['customer_name']
            phone_number = form.cleaned_data['phone_number']
            appointment_date = form.cleaned_data['appointment_date']

            # Barlag: öňki ýazgy bolmagy mümkin maglumatlar (Telefon belgisi, müşderiniň ady, we wagty)
            ...
            if existing_appointment:
                form.add_error(None, 'Bu stilist bilen bu wagty üçin öňden ýazgy edilişdir.')
            else:
                # Şol maglumatlar ýok bolsa, täze ýazgy saklanýar
    ...
    return render(request, 'appointment.html', {'form': form, 'stylist': stylist})
...
def stylist_detail(request, stylist_id):
    ...
    appointments = stylist.appointments.all()  

    return render(request, 'stylist_detail.html', {
        'stylist': stylist,
